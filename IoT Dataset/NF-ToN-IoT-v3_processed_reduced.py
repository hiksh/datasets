...
def format_network_flow_dataset(input_filepath, output_filepath):
    # 1. read CSV
    ...
    if 'Attack' in df.columns:
        rename_mapping['Attack'] = 'attack_name'
    if 'Label' in df.columns:
        rename_mapping['Label'] = 'attack_flag'
        
    # Apply changed column
    df.rename(columns=rename_mapping, inplace=True)
    
    # 3. if there is no attack_step column -> fill 0
...
if __name__ == "__main__":
    input_file = "NF-ToN-IoT-v3_processed_reduced.csv" 
    
    # new file name
    output_file = "Reformatted_NF-ToN-IoT-v3_processed_reduced.csv"
    
    format_network_flow_dataset(input_file, output_file)