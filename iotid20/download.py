...
KAGGLE_DATASET = "rohulaminlabid/iotid20-dataset"
INPUT_FILENAME = "IoTID20.csv"
OUTPUT_FILENAME = "Reformatted_IoTID20.csv"

# IoTID20 Cat → kill-chain step
KILL_CHAIN = {
    "normal": 0,
    "scan": 1,                 # Reconnaissance
    "mitm arp spoofing": 4,    # Exploitation — MITM
    "mirai": 6,                # C&C — Mirai botnet
    "dos": 7,                  # Actions on Objectives
}
...
def process(input_filepath, output_filepath):
    ...
    # Cat is the attack family; Sub_Cat is more granular
    df.rename(columns={"Cat": "attack_name"}, inplace=True)
    df.rename(columns={"Label": "attack_flag"}, inplace=True)
    df.drop(columns=["Sub_Cat"], errors="ignore", inplace=True)
