...
두 가지 다운로드 방법:
  ...
  방법 1 (즉시): Kaggle 라이선스 수락 후 kagglehub 사용
    → https://www.kaggle.com/datasets/chatzoglou/awid3
...
Usage examples:
  python3 download.py                           # Kaggle (라이선스 수락 필요)
  python3 download.py register NAME LAST EMAIL AFFIL   # 공식사이트 이메일 요청
  python3 download.py download-link https://...  # 이메일 링크로 직접 다운

Environment: 802.11 Wi-Fi SOHO testbed
Attacks: 13 categories — KRACK, Kr00k, deauth/disassoc flooding, beacon flood, etc.
"""
import kagglehub
import json
import os
...
# AWID3 label → kill-chain step (actual label strings verified against paper)
KILL_CHAIN = {
    "normal": 0,
    # Reconnaissance
    "probereqflood": 1,
    ...
    "authflood": 4,
    "arpattack": 4,
    # Actions on Objectives (DoS via forced disconnection)
    "deauthflood": 7,
    "disassocflood": 7,
    "powersavingdos": 7,
    "rtsflood": 7,
...
def register(name, lastname, email, affiliation, notes="Research purpose",
             website="", events="research", request_awid2=False):
    """Submit AWID3 dataset request to official site.
    ...
    print(f"[INFO] Submitting AWID3 request for: {email}")
...
def download_from_link(download_url):
    """Download AWID3 from the link received by email."""
...
def download():
    ...
    label_col = next((c for c in ["Label", "label", "Class", "class"] if c in df.columns),
                     df.columns[-1])
...
def process(input_filepath, output_filepath):
    ...
    label_col = next((c for c in ["Label", "label", "Class", "class", "attack_type"]
    ...
    # strip spaces and hyphens for normalization
    name_series = name_series.str.replace(r'[\s\-_]', '', regex=True)
