...
Data Cleansing Strategy (Resolving Label Contradictions):
    ...
    1. The "Unknown Attack" Anomaly: Rows with an attack flag (1) but a 'benign' category name.
       -> Resolution: Rename the attack_name to 'outlier' to prevent confusing the ML model.
    2. The "Silent Attack" Anomaly: Rows with an explicit attack category (e.g., 2.0, 3.0) but a 'benign' flag (0).
...
데이터 클렌징 전략 - 라벨 모순 해결 (Data Cleansing Strategy):
    ...
    1. "미분류 공격(Unknown Attack)" 이상치: 
       공격 플래그(attack_flag)는 1(공격)이지만, 공격명(Category, attack_name)이 'benign'로 기록된 모순된 행.
       -> 해결 방안: 머신러닝 모델의 학습 혼란을 방지하기 위해 해당 공격명을 'outlier'으로 강제 변경합니다.
...
def process_lspr23_dataset(input_filepath, output_filepath, chunk_size=100000):
    ...
    kill_chain_mapping = {
        ...
        # outlier → -1 (mapped via kill_chain_mapping, not via resolution logic)
        'outlier': -1,
    ...
    for chunk in reader:
        ...
        if 'Category' in chunk.columns:
            # Fill NaNs with 'benign'
            chunk['Category'] = chunk['Category'].fillna('benign')
            # Replace numeric 0 representations with 'benign'
        ...
        #          Only 'Label' carries the binary attack ground-truth (0 = benign, 1 = attack).
